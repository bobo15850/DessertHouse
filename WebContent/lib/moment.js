				...
				function() {
					...
					function localeMonths(m, format) {
						return isArray(this._months) ? this._months[m.month()]
								: this._months[MONTHS_IN_FORMAT.test(format) ? 'format'
										: 'standalone'][m.month()];
					}
					...
					function setOffsetToParsedOffset() {
						if (this._tzm) {
							...
							this.utcOffset(offsetFromString(matchOffset,
									this._i));
						}
						return this;
					}
					...
					function isSame(input, units) {
						var localInput = isMoment(input) ? input
								: local__createLocal(input), inputMs;
						if (!(this.isValid() && localInput.isValid())) {
							return false;
						}
					...
					function from(time, withoutSuffix) {
						if (this.isValid()
								&& ((isMoment(time) && time.isValid()) || local__createLocal(
										time).isValid())) {
					...
					function setWeekAll(weekYear, week, weekday, dow, doy) {
						...
						// console.log("got", weekYear, week, weekday, "set",
						// date.toISOString());
